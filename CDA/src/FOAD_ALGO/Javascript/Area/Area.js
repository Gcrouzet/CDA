...
class Area {
    ...
    */
     ...
     */
    addPoint(_point) {
        if (!(_point instanceof Point) || (this.pointArea.find(e => e.x === _point.x && e.y === _point.y) !== undefined)) {
            return false;
        }
        if (this.pointArea.length + 1 > this.areaSize) {
            return false
        }
        this.pointArea.push(_point);
        return true;
    }
